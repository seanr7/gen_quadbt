...
subplot(2,1,1)
semilogy(1:max_x, bst_hsvs(1:max_x), 'o','color',ColMat(1,:), LineWidth=1.5,MarkerSize=10)
hold on
...
disp('Frobenius norm error of the approximate BST HSVs; 200 nodes')
norm(diag(hsvbar_200(1:max_x))-diag(bst_hsvs(1:max_x)), "fro")
disp('Frobenius norm error of the approximate BST HSVs; 400 nodes')
norm(diag(hsvbar_400(1:max_x))-diag(bst_hsvs(1:max_x)), "fro")
disp('Frobenius norm error of the approximate BST HSVs; 800 nodes')
norm(diag(hsvbar_800(1:max_x))-diag(bst_hsvs(1:max_x)), "fro")
...
QBST_200_errors = zeros(testcases,1);    QBST_400_errors = zeros(testcases,1);
QBST_800_errors = zeros(testcases,1); 
for k = 1:testcases % orders to test
    ...
    QBST_ROM_200 = ss(Arbar_200, Brbar_200, Crbar_200, Drbar);
    QBST_ROM_400 = ss(Arbar_400, Brbar_400, Crbar_400, Drbar);
    QBST_ROM_800 = ss(Arbar_800, Brbar_800, Crbar_800, Drbar);

    BST_errors(k, 1) = norm(FOM-BRBT_ROM, 'inf')/sysnorm;
%     fprintf("Error in quadprbt")
%     norm(sys-sysrbar, 'inf')
    QBST_200_errors(k, 1) = norm(FOM-QBST_ROM_200, 'inf')/sysnorm;
    QBST_400_errors(k, 1) = norm(FOM-QBST_ROM_400, 'inf')/sysnorm;
    QBST_800_errors(k, 1) = norm(FOM-QBST_ROM_800, 'inf')/sysnorm;
end
