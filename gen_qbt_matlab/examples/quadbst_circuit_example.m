...
% 3-9 for moderate case
% nodes_40 = 1i*logspace(3, 9, N/2)';
...
disp('Frobenius norm error of the approximate BST HSVs; 40 nodes')
norm(diag(hsvbar_40(1:max_x))-diag(bst_hsvs(1:max_x)), "fro")
disp('Frobenius norm error of the approximate BST HSVs; 80 nodes')
norm(diag(hsvbar_80(1:max_x))-diag(bst_hsvs(1:max_x)), "fro")
disp('Frobenius norm error of the approximate BST HSVs; 160 nodes')
norm(diag(hsvbar_160(1:max_x))-diag(bst_hsvs(1:max_x)), "fro")
...
QBST_160_errors = zeros(testcases,1); 
for k = 1:testcases % orders to test
    ...
    QBST_ROM_160 = ss(Arbar_160, Brbar_160, Crbar_160, Drbar);
...
%     norm(sys-sysrbar, 'inf')
    ...
    QBST_160_errors(k, 1) = norm(FOM-QBST_ROM_160, 'inf')/sysnorm;
end
...
semilogy(2:2:2*testcases, QBST_160_errors,'-.r','color',ColMat(2,:),LineWidth=1.5);

lgd = legend('BST', 'QBST $(N = 40)$', 'QBST $(N = 80)$', 'QBST $(N = 160)$', 'interpreter','latex');
