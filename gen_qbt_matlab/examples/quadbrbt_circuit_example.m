...
disp("Implicit quadrature error in P; 40 nodes:")
norm(Utilde_40 * Utilde_40' - sampler.P, 2)/norm(sampler.P)
disp("Implict quadrarture error in Q; 40 nodes:")
...
disp('Frobenius norm error of the approximate BR HSVs; 40 nodes')
norm(diag(hsvbar_40(1:max_x))-diag(br_hsvs(1:max_x)), "fro")
disp('Frobenius norm error of the approximate BR HSVs; 80 nodes')
norm(diag(hsvbar_80(1:max_x))-diag(br_hsvs(1:max_x)), "fro")
disp('Frobenius norm error of the approximate BR HSVs; 160 nodes')
norm(diag(hsvbar_160(1:max_x))-diag(br_hsvs(1:max_x)), "fro")
...
QBRBT_40_errors = zeros(testcases,1);    QBRBT_80_errors = zeros(testcases,1);
QBRBT_160_errors = zeros(testcases,1); 
for k = 1:testcases % orders to test
    ...
    BRBT_ROM = ss(Ar, Br, Cr, Dr);
    QBRBT_ROM_40 = ss(Arbar_40, Brbar_40, Crbar_40, Drbar);
    QBRBT_ROM_80 = ss(Arbar_80, Brbar_80, Crbar_80, Drbar);
    QBRBT_ROM_160 = ss(Arbar_160, Brbar_160, Crbar_160, Drbar);

    BRBT_errors(k, 1) = norm(FOM-BRBT_ROM, 'inf')/sysnorm;
%     fprintf("Error in quadprbt")
%     norm(sys-sysrbar, 'inf')
    QBRBT_40_errors(k, 1) = norm(FOM-QBRBT_ROM_40, 'inf')/sysnorm;
    QBRBT_80_errors(k, 1) = norm(FOM-QBRBT_ROM_80, 'inf')/sysnorm;
    QBRBT_160_errors(k, 1) = norm(FOM-QBRBT_ROM_160, 'inf')/sysnorm;
end
...
semilogy(2:2:2*testcases, QBRBT_40_errors,'-.g<','color',ColMat(3,:),LineWidth=1.5);
semilogy(2:2:2*testcases, QBRBT_80_errors,'--mo','color', ColMat(4,:),LineWidth=1.5);
semilogy(2:2:2*testcases, QBRBT_160_errors,'-.r','color',ColMat(2,:),LineWidth=1.5);

lgd = legend('BRBT', 'QBRBT $(N = 40)$', 'QBRBT $(N = 80)$', 'QBRBT $(N = 160)$', 'interpreter','latex');
