...
QuadPRBTEngine = GeneralizedQuadBTReductor(sampler, modesl, modesr, weightsl, weightsr, "quadprbt")
for error in QuadPRBTEngine.quadrature_errors():
    print(error)

# Test Loewner Build
# MIMO
# Use SS quantities already built above
bt_sampler = QuadBTSampler(A, B, C, D=None)
# Default for reductor class is typ="quadbt"
QuadBTEngine = GeneralizedQuadBTReductor(bt_sampler, modesl, modesr, weightsl, weightsr)
Lbar, Mbar = QuadBTEngine.Lbar_Mbar
Gbar = QuadBTEngine.Gbar
Hbar = QuadBTEngine.Hbar
# Does L = L.T * U?
U_bar = bt_sampler.right_sqrt_fact_U(modesr, weightsr)
Lh_bar = bt_sampler.left_sqrt_fact_Lh(modesl, weightsl)
