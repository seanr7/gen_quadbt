...
class GeneralizedQuadBTReductor(object):
    ...
    @cached_property
    def sbar(self):
        # Compute and cache svs of Lbar (approximate hsvs)
        _, sbar, _ = np.linalg.svd(self.Lbar)
        return sbar

    @cached_property
    def Zbar(self):
        # Compute and cache approximate left svs of Lbar
        Zbar, _, _ = np.linalg.svd(self.Lbar)
        return Zbar

    @cached_property
    def Ybar(self):
        # Compute and cache approximate left svs of Lbar
        _, _, Ybarh = np.linalg.svd(self.Lbar)
        return Ybarh.H
    ...
    def reduce(self, r):
        ...
        Sbar1_invsqrt = np.diag(1 / np.sqrt(self.sbar[1:r]))
        Arbar = Sbar1_invsqrt @ self.Zbar[:, 1:r].H @ self.Mbar @ self.Ybar[:, 1:r] @ Sbar1_invsqrt
        Brbar = Sbar1_invsqrt @ self.Zbar[:, 1:r].H @ self.Hbar
        Crbar = self.Gbar @ self.Ybar[:, 1:r] @ Sbar1_invsqrt
